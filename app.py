...
def inject_custom_css(trade_state, theme):
    ...
    if theme == "Apple Light (Cupertino)":
        ...
        accent_neu = "#0071E3"
        accent_profit = "#34C759"
        accent_loss = "#FF3B30"
        nav_bg = "rgba(0,0,0,0.05)"
        nav_active_bg = "#1D1D1F"
        nav_active_text = "#F5F5F7"
        chart_template = "plotly_white"
        grid_col = "rgba(0,0,0,0.05)"
        blur_amount = "30px"
        
    elif theme == "Apple Dark (Pro)":
        bg_main = "#000000"
        bg_glass = "rgba(29, 29, 31, 0.72)"
        text_main = "#F5F5F7"
        text_secondary = "#86868B"
        ...
        accent_neu = "#D47A5A"
        accent_profit = "#336B4D"
        accent_loss = "#B34D40"
        font_heading = "'Source Serif Pro', serif"
        font_body = "'Inter', sans-serif"
        nav_bg = "rgba(0,0,0,0.04)"
        nav_active_bg = "#191919"
        nav_active_text = "#F0EBE1"
        chart_template = "plotly_white"
        grid_col = "rgba(0,0,0,0.05)"
        blur_amount = "15px"
        custom_body_css = f"""
        ...
        background-image: 
            linear-gradient(45deg, rgba(212, 122, 90, 0.05) 25%, transparent 25%, transparent 75%, rgba(212, 122, 90, 0.05) 75%, rgba(212, 122, 90, 0.05)),
            linear-gradient(45deg, rgba(212, 122, 90, 0.05) 25%, transparent 25%, transparent 75%, rgba(212, 122, 90, 0.05) 75%, rgba(212, 122, 90, 0.05));
        background-size: 40px 40px;
        background-position: 0 0, 20px 20px;
        animation: jiggleSquares 8s ease-in-out infinite alternate;
        """

    if trade_state == "PROFIT": accent_color = accent_profit
    elif trade_state == "LOSS": accent_color = accent_loss
    else: accent_color = accent_neu
    ...
    :root {{ 
        ...
        --accent-neu: {accent_neu}; 
        --text-main: {text_main}; 
        --text-secondary: {text_secondary}; 
        --font-heading: {font_heading};
        --font-body: {font_body};
        --nav-active-text: {nav_active_text};
        --fluid-black: {fluid_black};
        --fluid-text: {fluid_text};
        --fluid-border: {fluid_border};
    }}

    @keyframes jiggleSquares {{
        0%   {{ background-position: 0 0, 20px 20px; }}
        50%  {{ background-position: 5px 5px, 25px 25px; }}
        100% {{ background-position: -5px -5px, 15px 15px; }}
    }}
    ...
    .clay-panel {{ 
        ...
        padding: 24px; 
        backdrop-filter: blur({blur_amount}) saturate(180%);
        -webkit-backdrop-filter: blur({blur_amount}) saturate(180%);
    ...
    div[data-testid="stRadio"] label {{ 
        background: transparent !important; border-radius: 40px !important; 
        padding: 10px 24px !important; margin: 0 !important; cursor: pointer !important; 
...
def call_nexus_api(market_name, ticker, local_df, algo_name):
    ...
    if "Max Yield" in algo_name: multiplier, drift = 0.85, 0.02
    elif "Liquid Neural" in algo_name: multiplier, drift = 0.65, 0.01
    else: multiplier, drift = 0.50, 0.005
...
def fetch_real_news(ticker):
    try:
        res = requests.post(f"{API_URL}/api/v1/news", json={"ticker": ticker}, timeout=15)
        if res.status_code == 200: return res.json().get("news", [])
...
def generate_monte_carlo(df, days=5, simulations=100, algo_name=""):
    ...
    for t in range(1, days):
        simulated_paths[t] = simulated_paths[t-1] * (1 + np.random.normal(mu, sigma, simulations))
...
# Handle UI Avatar coloring dynamically based on theme
...
def get_fluid_figure():
    ...
    fig.update_layout(
        ...
        font=dict(color=t_main, family=f_heading), margin=dict(l=20, r=20, t=30, b=20), hovermode="x unified"
    )
    return fig
...
if selected_page == "Macro & Risk Engine":
    ...
    bb_line = 'rgba(128,128,128,0.4)'
    bb_fill = 'rgba(128,128,128,0.05)'
    up_col = "#34C759" 
    dn_col = "#FF3B30" 
