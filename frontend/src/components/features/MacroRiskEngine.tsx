...
export default function MacroRiskEngine({ stockData, prediction, currency, selectedAlgo, selectedMarket, selectedTicker }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="w-full space-y-5">
      <SectionHeader icon={<Activity className="text-accent w-4 h-4" />} title="Quantitative Metrics & Risk Modeling" />

      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <MetricCard title="Algo Target (T+1)" value={prediction ? `${prediction.predicted_price.toLocaleString(undefined, { minimumFractionDigits: 2 })} ${currency}` : "—"} sub={prediction ? `${prediction.pct_change >= 0 ? "+" : ""}${prediction.pct_change.toFixed(2)}% (Expected)` : ""} icon={prediction && prediction.pct_change >= 0 ? <TrendingUp className="w-4 h-4 text-emerald-400" /> : <TrendingDown className="w-4 h-4 text-red-400" />} />
        <MetricCard title="Historical Sharpe" value={sharpe.toFixed(2)} sub="Risk-Adjusted Alpha" />
        <MetricCard title="Value at Risk (95%)" value={`${var95.toFixed(2)}%`} sub="Max Daily Loss" icon={<TrendingDown className="w-4 h-4 text-red-400" />} />
        <div className="p-4 rounded-xl bg-surface border border-border flex flex-col justify-center items-center text-center transition-all hover:border-accent hover:shadow-[0_0_15px_var(--glow)]">
          <h4 className="text-lg text-accent font-bold tracking-tight">★ {prediction ? prediction.confidence.toFixed(1) : "—"}%</h4>
          <span className="text-[9px] text-foreground/40 uppercase tracking-widest mt-1">{prediction?.model_type || selectedAlgo}</span>
        </div>
      </div>

      <div className="p-5 rounded-xl bg-surface border border-border" style={{ height: 380 }}>
        <h3 className="text-[10px] tracking-widest text-foreground/40 uppercase mb-3">Monte Carlo Projection — {selectedAlgo.split("-")[0].trim()}</h3>
        <ResponsiveContainer width="100%" height="90%">
          <LineChart data={chartData}>
            ...
            <Line type="monotone" dataKey="historical" stroke="var(--foreground)" strokeWidth={2} dot={false} />
            <Line type="monotone" dataKey="aiTarget" stroke="var(--accent)" strokeWidth={3} strokeDasharray="5 5" dot={{ r: 5, fill: "var(--accent)" }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </motion.div>
...
function SectionHeader({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="flex items-center gap-2">
      {icon}
      <h2 className="text-[10px] tracking-[0.2em] text-foreground/50 font-bold uppercase">{title}</h2>
    </div>
  );
}
...
function MetricCard({ title, value, sub, icon }: any) {
  return (
    <div className="p-4 rounded-xl bg-surface border border-border transition-all hover:border-accent hover:shadow-[0_0_15px_var(--glow)]">
      <div className="text-[9px] tracking-widest text-foreground/40 uppercase mb-2">{title}</div>
      <div className="text-xl font-mono font-bold text-foreground mb-1 flex items-center gap-2">{value} {icon}</div>
      <div className="text-[10px] text-foreground/30">{sub}</div>
    </div>
  );
}
...
function LoadingState() {
  return (
    <div className="flex items-center justify-center h-64 text-foreground/30 font-mono text-sm animate-pulse">
