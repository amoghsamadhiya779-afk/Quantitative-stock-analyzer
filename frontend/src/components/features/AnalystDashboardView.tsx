        ...
        {prediction ? (
          <div className="flex flex-col gap-6 relative z-10">
            ...
            <div className="p-3 rounded-lg bg-black/20 border border-[var(--border)] text-xs text-[var(--foreground)]/70 font-mono leading-relaxed">
              Model outputs indicate a {Math.round(prediction.confidence * 100)}% probability of {prediction.forecast_direction} movement over the next 5 periods, conditioned on recent {stockData.volatility.toFixed(1)}% volatility regime.
            </div>
          </div>
        ) : (
